...
with open('Industry-account.txt','rb') as openfile:
...
# If-Idf Distance Initialization
#InitialTFIDF()
...
for industry_li in industry_list:
    ...
    while index_3 < len(cmp_list3) and index_2 < len(cmp_list2) :
        if  cmp_list3[index_3][5] < cmp_list2[index_2][5]:
            rowsarr_tmp.append(cmp_list3[index_3])
            index_3 = index_3 + 1
        else:
            rowsarr_tmp.append(cmp_list2[index_2])
            index_2 = index_2 + 1
    # check which one break firstly
    if index_3 == len(cmp_list3) - 1:
        for index_tmp in range(index_2,len(cmp_list2),1):
            rowsarr_tmp.append(cmp_list2[index_tmp])
    elif index_2 == len(cmp_list2) - 1:
        for index_tmp in range(index_3,len(cmp_list3),1):
            rowsarr_tmp.append(cmp_list3[index_tmp])
            
    #
    # merge 1 and (2,3)
    index_1 = index_2 = 0
    while index_1 < len(cmp_list1) and index_2 < len(rowsarr_tmp) :
        if  cmp_list1[index_1][5] < rowsarr_tmp[index_2][5]:
            rowsarr.append(cmp_list1[index_1])
            index_1 = index_1 + 1
        else:
            rowsarr.append(rowsarr_tmp[index_2])
            index_2 = index_2 + 1
    # check which one break firstly
    if index_1 == len(cmp_list1) - 1:
        for index_tmp in range(index_2,len(rowsarr_tmp),1):
            rowsarr.append(rowsarr_tmp[index_tmp])
    elif index_2 == len(rowsarr_tmp) - 1:
        for index_tmp in range(index_1,len(cmp_list1),1):
            rowsarr.append(cmp_list1[index_tmp])
    
    
    # start to scan all the data in rowsarr
    for row in rowsarr:
        sourcex = str(row[0])
    ...
    # testing rely on if similarity[13] == 1:
            if similarity[13] == 1:
                bayes_result = 'y'
            else:
                bayes_result = 'x'
            ...
            check_result = ""
            for check_id in range(2,14,1):
                check_result = check_result + str(similarity[check_id])+"," 
            check_result = check_result + sourcex+","+sourcey+","          
            check_result = check_result + str(bayes_result)
            print check_result
            with open('log.txt','a+') as logfile:
                logfile.write(check_result+"\n")
            if bayes_result == 'y':
                merge_flag = True
    ...
    # 2. Decistion Tree to classify
            ...
            '''   
    # 3. Set the weight to a certain threshold to detect the duplicate record
            '''
            const_value = -0.0868
            weight_list = [0.0764,0.1341,0.1965,0.0119,-0.1211,0.1516,0.1468,-0.0868,-0.0488,0.4525,0.2605,-0.0868]
            total = 0
            for cal_index in range(2,14,1):
                total = total + weight_list[cal_index]*similarity[cal_index]
            total = total + const_value
            weight_threshold = 0
            ...
            if merge_flag == True :
                merge_record.append(sourcex)
                merge_record.append(sourcey)
                                    
                # Data Fusion Step
                
                AS = 0.81298
                WordAccuracy = [0.0764,0.1341,0.1965,0.0119,0.1211,
                                0.1565,0.1468,0.0868,0.0488,0.4525,0.2605,0.0868]
                sameflag = similarity[1:13]
                prefer_value = BayesianFusion(AS,WordAccuracy,sameflag)
                #print prefer_value
                if prefer_value < 0.3:
                    trust = 1
                else:
                ...
                if trust == 2:
                    position_tmp=position_match
                    company_tmp=company_match
                    location_tmp=location_match
                    postdate_tmp=postdate_match
                    carlvl_tmp=carlvl_match
                    wrkexperience_tmp=wrkexperience_match
                    qualification_tmp=qualification_match
                    industry_tmp=industry_match
                    function_tmp=function_match
                    salary_tmp=salary_match
                    type_tmp=type_match
                    duty_tmp=duty_match
    
            if merge_flag == True :
                curposition_insert.execute("INSERT INTO position \
                (Source, Position, Company, \
                Location, Postdate, CareerLvl, \
                WrkExperience, Education, Industry, \
                Function, Salary, Type, Duty) VALUES \
                (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
                ('9',position_tmp,company_tmp,
